...
public class ObjectUsingCheckerRect : ObjectBase {
	...
	public ObjectData NewObjGet(GameObject cloneObj = null) {
        ...
        if(ObjectParent != null) {
            obj.ObjBody.transform.SetParent(ObjectParent.transform);
        }
        obj.isUsing = true;
        obj.NumberOfList = i;
        m_ObjList.Add(obj);
	...
	public void DeleteObjAll() {
        ...
        for(int i = 0; i < m_ObjList.Count; i++) {
            //Destroy(obj.ObjBody);
            //bool b = false;
            m_ObjList[i].isUsing = false;
            m_ObjList[i].ObjBody.SetActive(false);
        }
        //m_ObjList.Clear();
    }
